...
def cwt(signal, dt, dj=1/12, s0=-1, J=-1, wavelet='morlet', freqs=None):
    ...
    # bigger padding to try contain leakage
    base2 = np.fix(np.log(n0) / np.log(2) + 0.4999)
    nzeroes = (2 ** (base2 + 1) -n0).astype(np.int64)
    signal_ = np.concatenate((signal, np.zeros(nzeroes)))

    # Signal Fourier transform
    signal_ft = fft.fft(signal_, **fft_kwargs(signal_))
    N = len(signal_ft)
    # Fourier angular frequencies
    #ftfreqs = 2 * np.pi * fft.fftfreq(N, dt)
    # correct to reflect eqns 4 and 5 of Terrence and Compo:
    kplus = np.arange(1, int(N/2) +1)
    kplus = (kplus * 2 * np.pi / (N * dt))
    kminus = np.arange(1, int((N-1)/2) +1)
    kminus = np.sort((-kminus * 2 * np.pi / (N * dt)))
    ftfreqs = np.concatenate(([0.], kplus, kminus))
